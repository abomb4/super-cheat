...
const cannotMoveEffect = new StatusEffect("cannotMove");

cannotMoveEffect.speedMultiplier = 0;
cannotMoveEffect.effect = Fx.none;
...
const shieldConsumer = (paramEntity) => cons(trait => {
    if (trait.team != paramEntity.team
        && Intersector.isInsideHexagon(paramEntity.x, paramEntity.y, paramEntity.realRadius() * 2, trait.x, trait.y)) {
            trait.apply(cannotMoveEffect, 30);
    }
});
const COLOR = new Color(0, 200, 250, 1);
const blockType = extendContent(ForceProjector, "enemys-cannot-move", {});
