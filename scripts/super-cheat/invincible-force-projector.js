...
const shieldConsumer = (paramTile, paramEntity, paramBlock) => cons(trait => {
    if (trait.canBeAbsorbed() && trait.getTeam() != paramTile.getTeam() &&
        Intersector.isInsideHexagon(trait.getX(), trait.getY(), paramBlock.realRadius(paramEntity) * 2, paramTile.drawx(), paramTile.drawy())){
        trait.absorb();
        Effects.effect(Fx.absorb, trait);
        paramEntity.hit = 1;
    }
});
const blockType = extendContent(Block, "invincible-force-projector", {
    _timerUse: 0,
    ...
    update(tile) {
        ...
        Vars.bulletGroup.intersect(tile.drawx() - realRadius, tile.drawy() - realRadius, realRadius * 2, realRadius * 2, shieldConsumer(tile, entity, this));
    },
    realRadius(entity) {
