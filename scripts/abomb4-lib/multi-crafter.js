...
function defineMultiCrafter(config) {
    ...
    function initPlan(plan) {
        ...
        function drink(entity) {
            ...
            for (var i in consumeLiquids) {
                const consume = consumeLiquids[i];
                const liquid = consume.liquid;
                const use = Math.min(consume.amount * getProgressEfficiency(entity), entity.block.liquidCapacity);
        ...
        function doProduce(entity) {
            ...
            if (outputLiquids) {
                for (var i in outputLiquids) {
                    const output = outputLiquids[i];
    ...
    plans: [
        {
            ...
            output: {
                power: 100,
                items: [
                    { item: Items.sporePod, amount: 8 }
                ],
                liquids: [
                    { liquid: Liquids.slag, amount: 10 }
                ]
            },
            ...
            consume: {
                ...
                liquids: [
                    { liquid: Liquids.oil, amount: 1 }
                ]
            },
            output: {
                power: 100,
                items: [
                    { item: Items.surgeAlloy, amount: 10 }
                ],
                liquids: [
                    { liquid: Liquids.cryofluid, amount: 10 }
                ]
            },
